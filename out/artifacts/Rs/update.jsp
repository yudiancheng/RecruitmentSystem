...
<form id="frm" onsubmit="submitData()">
    <div class="main">
        ...
        <p class="short-input ue-clear">
            <label>*出生日期：</label>
            <input name="birthday" type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" value="${user.birth }"/>
        </p>
        ...
        <div class="short-input select ue-clear" id="position">
            <label>期望岗位：</label>
        </div>
        <p class="short-input ue-clear">
            <label>个人评价：</label>
            <textarea name="tip" >${user.tip}</textarea>
        </p>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        var $position = $("<select name='position' style='width:200px;height:30px;border:1px solid #C5D6E0'></select>");
        $("#xingbie").append($sex);
        ...
        d[8] = "软件工程师";
        var p_sex = "${user.sex}";
        var p_trade = "${user.major}";
        var p_salary = "${user.salary}";
        var p_position = "${user.desiredPosition}";
        //将当前项放到首位
        ...
        $.each(d,function(index,value){
            $("<option></option>").val(value).text(value).appendTo($($position));
        });
    });
